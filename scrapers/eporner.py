...
from tubescraper_addon.scraper_api.base import DirectScraper, DirectSource, DirectVideo
...
class EPornerScraper(DirectScraper):
    ...
    def resolve(self, video_id: str) -> list[DirectSource]:
        ...
        for link in tree.xpath("//a[contains(@href, '/dload/')]"):
            ...
            # A MEMBERS-ONLY rendition is listed exactly like the others --
            # same anchor, same "Download MP4 (1080p, h264, 1486.05 MB)" text,
            # the full size stated -- and is marked only by an onclick that
            # runs the site's login check before following the link. Without
            # this it ranks first on height and the player is handed the login
            # page, which arrives as 200 text/html and fails as a decode
            # error with nothing saying the file was never available.
            if "login" in (link.get("onclick") or ""):
                logger.debug(f"eporner: skipping members-only rendition on {video_id}")
                continue
