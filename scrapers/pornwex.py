"""pornwex.tv -- KVS tube indexing releases under their studio numbering.

One of the best title-matchers in this batch: a search for a library entry
comes back with the release itself ("Slut Puppies 18", 39:59) rather than a
clip cut out of it.

**This deployment scrambles its media URLs, and that is the whole difficulty
of the file.** ``video_url`` arrives as ``function/0/https://.../get_file/49/
<hash>/...``; stripping the prefix and requesting what is left returns a 404
from a URL that looks entirely reasonable. The first 32 characters of the hash
segment are permuted, and the permutation is derived from the page's own
``license_code`` -- see ``_unscramble``. Verified against this site: decoded,
the same video answers 206 ``video/mp4`` for a 388 MB file.

Because the scramble is undone with a value read from the page, ``resolve()``
must re-read the page every time; nothing here can be cached.

The helper is inlined rather than shared -- plugins must not import each
other (see this repo's AGENTS.md).
"""
...
class PornWexScraper(DirectScraper):
    key = "pornwex"
    name = "PornWex"
    base_url = "https://www.pornwex.tv"
        ...
        for link in tree.xpath("//a[contains(@href, '/video/')][@title]"):
            ...
            videos.append(
                DirectVideo(
                    ...
                    resolution=None,
                    views=parse_count(_first_text(link, "views")),
                )
            )
            if len(videos) >= limit:
                break
