"""saintporn.com -- small, recent KVS tube with genuinely 4K sources.

The narrowest catalogue in this batch, included because what it does carry is
high quality: cards state "4K" outright and the player offers real 720p/1080p
renditions, where most tube sites in this set state no height at all.

Unlike the numeric-id sites here, **a video is addressed by slug**
(``/video/tosh-s-creamy-curiosity/``), so ``video_id`` is that slug rather
than a number. Nothing downstream cares what the id looks like as long as
``resolve()`` accepts back exactly what ``search()`` emitted.

The ``qualtiy`` class name is the site's own misspelling, not a typo here.
Unlike a bare "HD" badge it does carry a real figure ("4K"), so it sets
``resolution`` as well as ``hd``.
...
_VIDEO_SLUG_RE = re.compile(r"/video/([^/?#]+)")
_HASH_LENGTH = 32
_MEDIA_PATH = "/get_file/"


class SaintPornScraper(DirectScraper):
    key = "saintporn"
    name = "SaintPorn"
