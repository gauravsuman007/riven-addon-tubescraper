"""watchporn.to -- KVS tube curating full scenes from premium studio sites.

The strongest site in this batch for *length*: it publishes whole scenes
rather than trimmed samples, so nearly every result runs past twenty minutes
and the files are large (a 1080p rendition here is routinely 400 MB+).

Two site-specific details:

**Every media URL carries a ``v-acctoken`` query parameter.** It is short
lived and issued per request for the page, which is why ``resolve()`` re-reads
the video page every time and nothing here may be cached -- a stored URL plays
for a while and then starts 403ing.

**Cards lazy-load their thumbnails.** ``src`` is an inline base64 spacer GIF
on every card and the real image is in ``data-original``; reading ``src``
gives a grid of identical blank tiles.

The ``function/0/`` unscrambler is inlined rather than shared because plugins
must not import each other (see this repo's AGENTS.md). This deployment
currently serves plain URLs, but that is a KVS setting that flips on upgrade
and fails as a silent 404 when it does.
"""
...
class WatchPornScraper(DirectScraper):
    key = "watchporn"
    name = "WatchPorn"
    ...
    def search(self, query: str, limit: int = 20) -> list[DirectVideo]:
        ...
        seen: set[str] = set()
        for link in tree.xpath("//a[@title and contains(@href, '/video/')]"):
            ...
            videos.append(
                DirectVideo(
                    ...
                    duration=parse_duration(_first_text(link, "thumb__info-item")),
                    # The card states no height, only a view count and a date.
                    resolution=None,
                    views=parse_count(meta[0].text_content() if meta else None),
                )
            )
            if len(videos) >= limit:
                break
