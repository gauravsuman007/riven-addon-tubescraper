"""whoreshub.com -- KVS tube aggregating full studio releases.

Search results here are titled the way the release is ("Slut Puppies 14 Alina
Lopez Gianna Dior"), which makes it one of the better sites in this batch for
matching a library entry by title *and* cast.

The one structural trap: **the view count lives outside the card's anchor.**
Each result is an ``<a class="item">`` followed by a sibling ``<ul class=
"info">`` holding views, rating and date. Anything that only walks the anchor
returns results with no view count at all, which is why the lookup below steps
out to ``following-sibling``.

Thumbnails are lazy-loaded (real image in ``data-src``, protocol-relative on
the ``wh.cdntrex.com`` CDN) and the card's ``is-hd`` badge is a claim with no
height attached, so it sets ``hd`` and never ``resolution``.

The ``function/0/`` unscrambler is inlined, not shared -- plugins must not
...
class WhoresHubScraper(DirectScraper):
    key = "whoreshub"
    name = "WhoresHub"
    ...
    def search(self, query: str, limit: int = 20) -> list[DirectVideo]:
        ...
        for link in tree.xpath(
            ...
            thumbnail = ""
            if images:
                thumbnail = images[0].get("data-src") or images[0].get("src") or ""
                if thumbnail.startswith("data:"):
                    thumbnail = ""
                elif thumbnail:
                    thumbnail = urljoin(self.base_url + "/", thumbnail)

            # Views sit in the sibling <ul class="info">, not inside the card.
            counts = link.xpath(
                "following-sibling::ul[contains(@class, 'info')]"
                "//span[contains(concat(' ', normalize-space(@class), ' '), ' text ')]"
            )

            title = (link.get("title") or _first_text(link, "description")).strip()
