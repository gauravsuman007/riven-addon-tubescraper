"""x-x-x.tube -- KVS tube whose catalogue leans to whole "full XXX films".

Useful in this set because it indexes releases under their studio numbering
("Slut Puppies 18_4"), so a library title often matches directly.

Two site-specific details:

**Search is a path, not a query string.** ``/search/<slug>/`` where the slug
is the query lowercased with runs of non-alphanumerics collapsed to "-". A
``?q=`` on this template renders the unfiltered front page rather than an
error, so a wrong URL here looks like "the site returned irrelevant results"
instead of a 404.

**The visible card title is truncated** with an ellipsis ("Watch the full XXX
films of the Slut Puppies 18_4 vi..."). The anchor's ``title`` attribute holds
the untruncated text, and a clipped title is worse than useless to the ranker
that scores these against a library entry, so the attribute wins.

The ``function/0/`` unscrambler is inlined rather than shared, because plugins
must not import each other (see this repo's AGENTS.md).
"""
...
class XXXTubeScraper(DirectScraper):
    key = "xxxtube"
    name = "X-X-X.tube"
    ...
    def search(self, query: str, limit: int = 20) -> list[DirectVideo]:
        ...
        seen: set[str] = set()
        for link in tree.xpath("//a[contains(@href, '/videos/')][@title]"):
            ...
            videos.append(
                DirectVideo(
                    ...
                    # The attribute, not .title: the rendered text is clipped.
                    ...
                    resolution=None,
                    views=parse_count(_first_text(link, "rating")),
                )
            )
            if len(videos) >= limit:
                break
