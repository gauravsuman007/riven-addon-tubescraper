"""Tests for the tube-scrape framework: matching, ranking, and plugin discovery.

Every site scraper -- parsing one site's markup, resolving a video's playable
sources -- lives in this repository's `scrapers/` folder and is loaded from
there at runtime. What is tested here is what genuinely belongs to no single
site: the shared parsing helpers in `base.py`, the matching and ranking every
scraper's results run through, and the plugin-loading mechanism itself.

Every case corresponds to a defect a live flow check across the library
actually turned up, or to an assumption that would silently degrade results
rather than fail loudly.

Run it the way the host's own suites are run -- a plain script, not pytest:
...
# only while `riven_addon.py` runs. A test invoked directly has to do the same
# thing for itself, or every import below fails with ModuleNotFoundError and
# it looks like the add-on is broken rather than the test being run oddly.
...
try:
    ...
    # House style: a suite whose dependencies are absent prints why and exits
    # 0. These modules need loguru and the host's settings package, which are
    # present inside the container and generally not on a laptop -- and a
    # suite that fails purely because of where it was run teaches people to
    # ignore red.
...
#
# The whole point of bundling them is that installing the add-on installs the
# scrapers and updating it refreshes them. If that folder stops loading, the
# add-on is present, its settings tab renders, its API answers -- and every
...
check(
    ...
    len(bundled.plugins) >= 20,
    f"found {len(bundled.plugins)}",
)
check(
    "no bundled scraper fails to import",
    bundled.errors == {},
    str(bundled.errors),
)
check(
    "every bundled scraper declares a distinct key",
