...
# `direct_service()` is the process-wide singleton (`tubescraper_addon.service`,
...
def direct_search_stream(
    ...
    per_site = (
        ...
        else config.settings().results_per_site
    )
    ...
    return StreamingResponse(
        ...
        headers={
            ...
            # single delivery at the end. Set here rather than in the
            # frontend's proxy because that proxy is generic to every add-on
            # and has no way to know which responses are event streams.
...
def _plugins_response() -> PluginsResponse:
    return PluginsResponse(
        ...
        site_order=list(config.settings().site_order),
        scrapers=[
            ScraperInfoModel(
                key=info.key,
                name=info.name,
...
def direct_plugin_set_enabled(
    ...
    Written to this add-on's `disabled` list directly rather than through
    the generic settings form -- that field is hidden from the rendered
    schema for the same reason `tailscale.auth_key` is: a second write
    path to the same value with no way to tell which one is current is
    exactly the "two auth key fields" bug repeating itself.
    """
...
def direct_plugins_set_order(body: SiteOrderBody) -> PluginsResponse:
    ...
    Written straight to this add-on's `site_order` rather than through the
    generic settings form, for the same reason `disabled` is.
