...
def describe_scrapers() -> list[ScraperInfo]:
    ...
    # Imported here, not at module scope: `config` reaches the host's
    # settings, which pull in a large part of the application (RTN, the
    # DB models, ...), and this module is imported by test suites that
    # stub those out deliberately.
    from tubescraper_addon import config

    disabled = set(config.settings().disabled)
    infos: list[ScraperInfo] = []

    discovery = discover_plugins(config.plugin_dir())
...
class DirectScraperService:
    ...
    #:
    #: These are only the FALLBACK. `site_order`, set from the
    #: add-on's own controls, takes precedence when it is non-empty -- see
    ...
    def _load_all(self) -> dict[str, DirectScraper]:
        ...
        Every site scraper is a plugin, and the maintained set ships in
        this add-on's own `scrapers/` folder -- so installing the add-on
        installs them and updating it refreshes them, with no separate
        repository or mounted folder to keep in step.
        """
...
def site_tier(key: str) -> int:
    ...
    order = config.settings().site_order
